...
/**
 * 측정기 목록 조회 + 캐싱(cacheTag: 'air-devices').
 *
 * 실패 시 의도적으로 throw한다 — 'use cache' 스코프 안에서 던진 에러는 캐시에 저장되지
 * 않으므로, 백엔드 일시 장애가 cacheLife 기간 동안 빈 목록으로 굳는 것을 막는다.
 * 폴백은 캐시 밖(getAirDeviceListServer)에서 처리한다.
 */
async function fetchAirDeviceList(): Promise<AirDevice[]> {
  'use cache';
  cacheLife('default');
  cacheTag(AIR_DEVICES_CACHE_TAG);

  const { data } = await axios.get<AirDevice[]>(
    `${getApiBaseUrl()}/air-devices`,
    { timeout: 10000 },
  );
  return data;
}
...
/**
 * 공개 측정기 목록을 가져온다(서버 컴포넌트 전용).
 * GET /air-devices — published=true만, order ASC 서버 정렬. items도 order ASC.
 *
 ...
 * 소비 뷰는 빈 배열을 정상 케이스로 처리해 폴백 UI를 렌더한다.
 */
export async function getAirDeviceListServer(): Promise<AirDevice[]> {
  try {
    return await fetchAirDeviceList();
  } catch {
    return [];
  }
}
