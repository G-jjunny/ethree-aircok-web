...
import {
  useDeleteDiagnosisImageMutation,
  useReorderDiagnosisImagesMutation,
} from '@/features/diagnosis-image-editor'
import type { DiagnosisImage } from '@/entities/diagnosis-image'
import { SortableDiagnosisImageCard } from './SortableDiagnosisImageCard'

interface DiagnosisImageGridSectionProps {
 ...
 * diagnosis-image는 백엔드에 교체(PATCH:id) 엔드포인트가 없어 교체 기능은 제공하지 않는다.
 */
export function DiagnosisImageGridSection({
  images,
  isLoading,
}: DiagnosisImageGridSectionProps) {
  ...
  const [items, setItems] = useState<DiagnosisImage[]>(images)
  const [syncedFrom, setSyncedFrom] = useState<DiagnosisImage[]>(images)
  ...
  const deleteMutation = useDeleteDiagnosisImageMutation()
  const reorderMutation = useReorderDiagnosisImagesMutation()
