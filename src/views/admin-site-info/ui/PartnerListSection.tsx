  ...
  return (
    <section className="bg-surface-white rounded-xl border border-border-light p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h2 className="text-nav font-display font-semibold text-heading-dark">파트너 & 고객사</h2>
        <button
          type="button"
          onClick={() => setIsAddModalOpen(true)}
          className="bg-aircok-blue text-heading-light rounded-md px-4 py-2 min-h-[44px] text-nav font-medium hover:bg-aircok-blue-dark active:scale-[0.97] transition-colors"
        >
          {tabLabel} 추가
        </button>
      </div>
          ...
          <button
            ...
            className={[
              ...
              activeTab === tab
                ...
                : 'bg-surface-light text-body-dark hover:bg-border-subtle',
            ].join(' ')}
          >
      ...
      {tabPartners.length === 0 ? (
        <p className="text-secondary-dark text-sm py-6 text-center">등록된 {tabLabel}이 없습니다.</p>
      ) : (
        <ul className="flex flex-col gap-3">
          {tabPartners.map((partner, index) => (
            ...
            >
              {/* 로고 */}
              <div className="w-12 h-12 rounded-md overflow-hidden flex-shrink-0 bg-surface-white border border-border-light flex items-center justify-center">
                {partner.logoUrl ? (
                  ...
                  <span className="text-secondary-dark text-xs">로고 없음</span>
                )}
              </div>

              {/* 이름 + 순서 */}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-heading-dark truncate">{partner.name}</p>
                <p className="text-xs text-secondary-dark">순서 {partner.order}</p>
              </div>
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-md text-xs font-medium text-body-dark bg-surface-white border border-border-light hover:bg-surface-light disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
                >
                  로고
                </button>
