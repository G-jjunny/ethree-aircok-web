...
- **API 레이어 규칙 검토** (CLAUDE.md "API 통신 규칙"): ① `fetch()` 직접 호출이나 `useEffect`+`useState` 데이터패칭이 남아 있지 않은지, ② widgets/views가 `axiosInstance`를 직접 import하지 않고 entities/features를 경유하는지, ③ 도메인 API가 `status`/`isAuthError`/`parseAxiosMessages`/retry 로직을 복붙 재정의하지 않고 `shared/api`의 `ApiError`/`authAwareRetry`/`parseAxiosMessages`를 import하는지 확인한다. 위반 시 frontend-implementer에게 반려한다.
