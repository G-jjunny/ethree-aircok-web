...
**Next.js:**
- `app/` 디렉토리가 이미 존재하면 FSD의 app 레이어는 `src/app/`으로 통합
- Next.js App Router 사용 시 라우팅은 Next.js 방식 유지
- FSD에서 "pages" 대신 "views" 사용 (Next.js pages 라우터와 충돌 방지)

**Vite:**
- `vite.config.ts`에 alias 설정 추가

5. `eslint.config.mjs`에 FSD import 규칙(eslint-plugin-boundaries) 추가 권장사항 안내
