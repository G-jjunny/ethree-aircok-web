...
async function main() {
  ...
  if (diagnosisCount > 0) {
    console.log(
      `DiagnosisImage already seeded (${diagnosisCount} records), skipping.`,
    );
  } else {
    const result = await prisma.diagnosisImage.createMany({
      data: seed.diagnosisImages.map((item) => ({
        imageUrl: item.imageUrl,
        order: item.order,
      })),
    });
    console.log(`DiagnosisImage seeded: ${result.count} records inserted.`);
  }
