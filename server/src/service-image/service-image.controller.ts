...
// 서비스 이미지 업로드 허용 MIME 타입 화이트리스트 (#83). 이미지 전용.
...
export class ServiceImageController {
  ...
  /** GET /api/service-images — 공개 엔드포인트. order ASC, createdAt ASC 정렬. */
  @Get()
  findAll() {
  ...
  /**
   * POST /api/service-images — JWT 인증 필요. 201 Created.
  ...
  async create(@UploadedFile() file: Express.Multer.File) {
    ...
    const url = await this.r2Service.upload(file, 'service-images');
    return this.serviceImageService.create(url);
  }

  /**
   * PATCH /api/service-images/reorder — JWT 인증 필요.
   * 주의: 반드시 PATCH /:id 보다 먼저 선언해야 한다.
   * 그렇지 않으면 NestJS 라우터가 'reorder' 를 :id 로 매칭한다.
   */
  @UseGuards(JwtAuthGuard)
  @Patch('reorder')
  reorder(@Body() dto: ReorderServiceImagesDto) {
    return this.serviceImageService.reorder(dto);
  }

  /** DELETE /api/service-images/:id — JWT 인증 필요. 204 No Content. */
  @UseGuards(JwtAuthGuard)
  @Delete(':id')
