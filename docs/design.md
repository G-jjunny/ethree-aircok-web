...
### Contact Split Section (문의 폼 + 지도 2단 레이아웃)

`/contact` 페이지의 "온라인 문의" 영역. 데스크탑(`lg` 이상)에서는 **좌측 문의 폼 / 우측 지도**를 가로 2단으로 배치하고, `lg` 미만(모바일·태블릿)에서는 폼이 위·지도가 아래로 가는 세로 스택으로 폴백한다. 기존에 분리돼 있던 폼 섹션(`bg-surface-white`)과 지도 섹션(`bg-surface-light`)을 **하나의 섹션으로 통합**해 단일 배경 위에서 2단으로 배치하며, 폼과 지도를 동일한 카드 스타일로 묶어 통일감을 준다.

> 신규 색상·radius·shadow·spacing 토큰은 추가하지 않는다. 모두 기존 토큰(`bg-surface-white`, `bg-surface-light`, `rounded-xl`, `shadow-card`, `border-border-light`, `gap-8`/`gap-10` 등)의 조합으로 구성한다.

**섹션 배경 통일**
- 통합 섹션 배경: `bg-surface-light` (정보성 섹션 톤 유지 — 라이트 그레이 위에 흰 카드가 떠 보이도록)
- 섹션 패딩: `py-20` (기존 폼/지도 섹션과 동일)
- 폼·지도 두 카드 모두: `bg-surface-white rounded-xl shadow-card`
  - 그림자는 `shadow-card` 단일만 사용(§6 그림자 철학 준수). 카드 테두리는 쓰지 않는다(지도 카드도 `border` 대신 `shadow-card` + `overflow-hidden`으로 정리).

**섹션 헤더 처리 (상단 공통 헤더 권장)**
- 2단 가로 배치에서 좌우 칼럼 위에 중앙정렬 헤더가 뜨면 어색하므로, `SectionHeader`를 **그리드 위 상단 공통 헤더로 1개만** 배치한다.
- `align`은 좌측 정렬(`align="left"` = 기본값)을 권장 — 2단 레이아웃의 좌측 칼럼(폼) 시작선과 헤더 시작선을 맞춰 정돈된 인상을 준다. (기존 `align="center"`에서 변경)
- 헤더와 그리드 사이 간격: `mt-10` 또는 `mt-12`
- `label="문의하기"`, `title="온라인으로 문의하기"`, `theme="light"`, `titleAs="h2"` (콘텐츠·카피 변경 없음)

**그리드 구조**
- 그리드 래퍼: `grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-stretch`
  - `lg:grid-cols-2`로 PC에서 좌우 2단, 그 미만은 1열 세로 스택
  - `items-stretch`로 두 칼럼이 같은 높이로 늘어나도록 해 폼·지도 높이를 정렬
  - `gap-8`(32px) → `lg:gap-10`(40px)로 데스크탑에서 칼럼 간 여백 확대

**폼 칼럼**
- 카드 래퍼: `bg-surface-white rounded-xl shadow-card p-6 sm:p-8`
- 폼 자체 최대 너비 제한(`max-w-[640px] mx-auto`)은 2단에서 칼럼 폭이 이미 적정하므로 **제거**한다. 1열 스택(모바일·태블릿) 상태에서 폼이 과도하게 넓어지는 것을 막으려면 카드 래퍼에 `lg:max-w-none max-w-[640px] mx-auto lg:mx-0`를 적용해, `lg` 미만에서만 640px 가독 폭을 유지하고 `lg`에서는 칼럼 전체를 채우게 한다.
- `InquiryForm` 내부 마크업은 변경하지 않는다(features 슬라이스 책임).

**지도 칼럼**
- 카드 래퍼: `bg-surface-white rounded-xl shadow-card overflow-hidden`
- 지도 높이: 2단 배치에서 `aspect-video`(16/9) 고정비는 폼 높이와 어긋날 수 있으므로, **칼럼 높이에 맞춰 늘어나도록** 처리한다.
  - 카드 래퍼에 `h-full` + 최소 높이 보장 `min-h-[360px]` {/* token 없음: 지도 최소 높이, 1회성 레이아웃 수치 */}
  - iframe: `w-full h-full border-none` 유지
  - 1열 스택 상태에서도 `min-h-[360px]`가 적용돼 지도가 너무 납작해지지 않는다. (기존 `aspect-video`는 제거)
- iframe `title`·`src`·접근성 속성은 변경하지 않는다.

```tsx
// Contact Split Section 예시 (폼 + 지도 통합 2단)
<section className="bg-surface-light py-20">
  <div className="content-container">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-stretch mt-10">
      {/* 폼 카드 */}
      <div className="bg-surface-white rounded-xl shadow-card p-6 sm:p-8 max-w-[640px] mx-auto lg:max-w-none lg:mx-0 w-full"> {/* token 없음: 폼 가독 폭 640px (lg 미만 전용) */}
        <InquiryForm />
      </div>
      {/* 지도 카드 */}
      <div className="bg-surface-white rounded-xl shadow-card overflow-hidden h-full min-h-[360px]"> {/* token 없음: 지도 최소 높이 */}
        <iframe
          title="본사 위치 지도"
          src={mapSrc}
          loading="lazy"
...
> **구현 메모**: 이 패턴은 기존 `ContactFormSection` + `ContactMapSection`을 하나의 섹션 컴포넌트로 통합하는 것을 전제로 한다. 단, views 슬라이스 섹션 분리 규칙상 두 파일을 그대로 두고 `ContactView`에서 묶어 렌더하고 싶다면, 두 섹션의 `<section>` 배경/패딩을 제거하고 공통 래퍼 섹션(`bg-surface-light py-20` + `content-container` + 그리드)을 새로 두는 방식도 허용한다. 어느 쪽이든 **배경은 `bg-surface-light` 단일로 통일**한다.
